...
namespace AnimationImporter
{
	public class AnimationImporterWindow : EditorWindow
	{
		...
		{
			GUILayout.Label("Object Path example: Child/ChildOfChild");

			GUILayout.BeginHorizontal();

			if(importer.useFileNameAsPath)
			{
				GUI.enabled = false;	
				GUILayout.TextField(string.Empty, GUILayout.MaxWidth(300f));	
				GUI.enabled = true;
			}
			else
			{
				importer.objectPath = GUILayout.TextField(importer.objectPath, GUILayout.MaxWidth(300f));
			}

			importer.useFileNameAsPath = GUILayout.Toggle(importer.useFileNameAsPath, "File Name as Path");
