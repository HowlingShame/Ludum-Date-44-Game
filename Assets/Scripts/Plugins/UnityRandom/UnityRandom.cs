...
public class UnityRandom 
{
	...
	// RANGE Return a int min <= x <= max
	public int RangeInclusive(Int32 minValue, Int32 maxValue)
	{
	...
	{
		return (float)((Double)_rand.NextUInt32() / ((UInt64)UInt32.MaxValue)) * (maxValue - minValue) + minValue;
	...
	{
		return (float)((Double)_rand.NextUInt32() / ((UInt64)UInt32.MaxValue + 1)) * (maxValue - minValue) + minValue;
	}
	...
	// RANDOM from animatio curve position
	...
	// uses unity random function, moves every member to random postion
	static public void RandomizeList<T>(List<T> source)
	{
		int n = source.Count;  
		while(n > 1) 
		{
	...
	static public void RandomizeList<T>(List<T> source, int swapsCount)
	{
		for(int n = 0; n < swapsCount; ++ n)
		{
			int c = UnityEngine.Random.Range(0, source.Count);
			int t = UnityEngine.Random.Range(0, source.Count);
